...
- 🎓 **AI 导师（2.0 的入口页）**：手机本地用规则算出「发现 + 依据 + 动作」，每条结论都能指到具体数字；今日任务卡（落库、可跳转、可打勾回填）；可对话（会话带结构化证据包，禁止编造）；缺画像就当场追问并即问即存
- 📚 **材料中心（2.0）**：由软件供给**真实材料** —— 外刊/新闻 RSS、Project Gutenberg 公版书全文、播客/TED、Wikipedia/arXiv（全部无需 API Key）；打开即**本地算难度**（已知词覆盖率 / 生词密度 / CEFR / 预计时长）；阅读器点词即查、一键收词入复习队列、进度落库；也可粘贴自备材料
- 📝 **读后测验（2.0）**：从材料原文本地生成完形填空与中译英回想（零 API、可复现），结果进学习档案，错词回到复习队列
- 🧭 词汇量测试（2.0）：速测 5 分钟 / 完整版 10 分钟（含语法与阅读小题），伪词校准自评偏差，给出区间与「你在哪一档开始掉」
- 🧠 学习画像与模型：每个字段都带**来源（自报/测量/推断）+ 置信度**，不再是"收藏了多少词就是什么水平"
- 📊 学习统计：学习曲线、热力图、分类计数、连续天数；写译记录与练习日志
