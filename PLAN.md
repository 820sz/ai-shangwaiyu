...
## 状态
- 当前版本:**v2.4.0+61**(Release v2.4.0 = Latest;上版 v2.3.3)
- 当前阶段:🔧 **按用户实测反馈逐条打磨**(用户 2026-09-24 一次性提了 10 条问题,本版全部落地;任务与边界见 `docs/TASK-BOARD-2026-09-24.md`)。**用户明确要求:只做他列的事,不再自作主张加功能;跟读与每日提醒都不需要**
- 状态协议:IN_PROGRESS(等用户真机复测 v2.4.0 的 10 项;当前分支 analyze 0 error/0 warning、687 测试全绿 +1 skip、v2.4.0 已发 Release)
...
### v2.4.0 — 用户实测反馈 10 条全部落地(2026-09-24,已发布)
**背景**:v2.3.3 发出后用户在真机上一次性反馈 10 个问题(识图精度、AI 补全没法用、追问复制、浅色看不清、归类不生效、词汇原型/出现次数、翻译生硬、助理对话要独立窗口、材料中心全是 AI 二手改写…),并明确要求:**只做他列的事、不要自作主张加功能、动手前先问清楚**。本版逐条交付,顺序与边界见 `docs/TASK-BOARD-2026-09-24.md`。
**交付(按用户编号)**:
1. **识图**(最大痛点):识图**固定低思考档**(实测"高"档一次 6 秒 + 6225 字思考只出 1 条,思考挤占正文且容易跑偏);图片**按 EXIF 转正 + 长边压 2000px 统一 JPEG**(样本里有横置/倒置页);提示词加 4 条实战纪律(任意方向先转正、**一个词就是 word**、跨行标记接完整、新增 `line` 证据行);新增**本地确定性校验层**(`vision_guard`:重复/碎片/截断/类型重判/证据核对);结果头部显示「已校验:忽略 N 条 · 纠正类型 M 条」并可展开看每条原因 —— 把"盲盒"打开。
   **纠正**:手写中文批注**保留**(用户当场指出要靠它补录),不再当垃圾丢。
2. **AI 补全没法用**:根因是追问端点未配置时按钮被静默禁用(灰且无解释);现始终可点、缺配置明确提示并可一键跳设置、追问端点未配时回落主 API。
3. **追问复制**:自定义菜单曾把系统默认「复制」换掉 → 选中一段复制不了那段;现保留平台默认项(复制/全选/分享),补「复制整条」「取消选择」。
4. **词汇原型备注**:`taming(tame)` —— 规则 + 不规则表推导,**候选必须是词频表里的真词才显示**;生词本与词条详情显示。
5. **出现次数与出处**:同词不再重复入库,而是**合并一次出现**(书+页+句去重),显示 `apple(×2)`,词条详情新增「出现记录(N 次)」逐条列出出处与原句;新增 `occurrences_json` 列(**dbVersion 12**);既有释义/音标不被覆盖。
6. **翻译质量**:识图与补全提示词都要求结合语境判词性、不给词典第一义项、整句通顺不硬译。
7. **助理独立对话窗口** `tutor_chat_screen.dart`:AI 头像、流式、思考可折叠、**思考强度可选**、读取全部本地数据、消息落库;学习助理页只留任务与诊断 + 入口卡。同时修「今天做什么」**划掉后可取消完成**(新增 `reopenTutorTask`)。
8. **归类不生效**:两个根因 —— ①取消子分类对话框仍会执行移动(分类改了、路径没改)②材料名留空拼出只有一级的伪路径;现取消=整件事取消,路径统一由 `buildMaterialPath` 构造(空名落「未命名材料」),并用选中词的共同出处预填。
9. **浅色看不清**:实测描边对白底 1.70:1、输入框 1.61:1(要求 3:1);主题层加深描边(5.2:1 / 3.4:1)、输入框与 chip 补可见描边、去掉自行调透明度的文字色;新增**浅色对比度闸门测试**。
10. **材料中心**:①「按你的水平找材料」改为**方向自选**,默认**「资料原文」**(Gutendex/arXiv/RSS 真实原文检索,可点开考究),「AI 整理编写」单独一栏并加黄色警示 + 条目「AI 编写」角标;②AI 推荐新增**「再多来几条」**(追加、按标题去重、全重复时明确提示)。
**新增测试(63 例)**:识图校验 16、图片预处理 7、出现记录 9、原型备注 11、快捷回复 6、原文检索 6、归类修复 6、主题对比度 2
**验证**:analyze **0 error / 0 warning**;**687 测试全绿 + 1 skip**(v2.3.3 为 624);APK 2.4.0+61 arm64,sha256 `810cb1d8…6110` 与 Release asset `digest` 逐字一致

### v2.3.3 — 生词去重 + 复习/听写链路整链路测试(2026-09-24,已发布)
**背景**:延续 v2.3.2 的"整链路测试"打法,这轮给**复习链路**与**听写链路**各补一条端到端测试(真 SQLite,逐句复刻界面里的调用序列),又抓出一个真 bug。
**修复**:
- **同一个词被收进生词本两次**:`insertVocabularies` 是裸插入、零查重 —— 听写页「把没听出的词收进生词本」成功后**按钮没禁用**,点第二次就把同一批词插第二遍;一批里两句漏同一个词、AI 识图返回重复词也都会重复;结果是生词本一个词两行、复习队列里同一张卡出现两次(还互相争配额)。修法放在**数据库层**一次修好(所有收词入口都走这里,比在四个界面各写查重可靠):插入前把"库里已有 + 本批已处理"的词过一遍,词面按 `trim().toLowerCase()` 比较,重复跳过,返回**真正插入的条数** —— 与备份恢复"按词面去重、已存在跳过"的既有约定一致
- 听写页:保存成功后按钮变「已收进生词本」并禁用;提示拆成"新增 N 个 / 本来就在 M 个 / 挂上复习状态 K 个"
**新增测试(`test/review_flow_test.dart`,6 例)**:复习链(评「不认识」→ 到期时间前移、lapses 累加、掌握度回退、错误档案留痕、当天不再重复出它;新词评「认识」→ 不再是新词、稳定度真的写入;7 天负荷与到期时间对得上)+ 听写链(抽句 → 漏词判分 → 一键收词 → **真的进今日队列**;重复收只插一次;批内重复只插一条)
**验证**:analyze **0 error / 0 warning**;**624 测试全绿 + 1 skip**(v2.3.2 为 618);APK 2.3.3+60 arm64,sha256 `beb33d4b…97f7` 与 Release asset `digest` 逐字一致

### v2.3.2 — 整链路测试挖出的三个真 bug(材料库)(2026-09-24,已发布)
**背景**:给"材料 → 阅读 → 测验 → 错词进错误档案 → 复习队列 → 学习助理诊断"这条主路径补了一个**端到端测试**(真 SQLite),一跑就抓出三个此前所有单测都覆盖不到的缺陷 —— 单测各自全绿,但用户真机走的整条路是断的。
**修复**:
- **材料库条目 id 恒为 0**(点击打不开):SQL 只输出 `material_id`,而 `ShelfItem` 读 `id` → 每条 id 都是 0,点书架里任何一份材料都去打开"id=0";现同时带出 `id`,并补齐 `source`/`url`/`audio_url`。该缺陷自 v2.0 起存在,因书架常为空而未被发现
- **入库但没打开过的材料在材料库里不可见**:查询是 `material_progress` INNER JOIN `materials`(从有进度的行出发),于是"粘贴入库 → 这次没读 → 回到材料中心显示还没有材料",用户以为导入失败;改为 `materials LEFT JOIN material_progress`,排序按 COALESCE(读过的按最近阅读、没读的按入库时间)
- **学习助理可能被一次 Hive 读失败整页炸掉**:快照加载器承诺"任何失败都降级",但读学习者模型那一步没包降级;现改为读不到就用空模型继续
- 文案:`按 你的自评:9000 词` → `基线:你的自评 9000 词`
**新增整链路测试(`test/learning_flow_test.dart`,4 例)**:材料入库(难度分析)→ 书架可见 → 收词/进度/会话 → 标记读完 → 读后测验出题判分 → 错词进错误档案 + 复习状态回退 → **今日队列真排得出该词** → 诊断出结论且每条带依据;外加坏 `difficulty_json` 容错、配额"只复习不加新词"两点边界
**同批修正**:`db_v2_queries_test` 里那条"没有进度的材料不算读过"的断言**把 bug 锁死了**,已按新语义重写(并新增"id 必须带出来""没读过时进度为空而非 0"的断言)
**验证**:analyze **0 error / 0 warning**;**618 测试全绿 + 1 skip**(v2.3.1 为 614);APK 2.3.2+59 arm64,sha256 `92e36696…305d` 与 Release asset `digest` 逐字一致

### v2.3.1 — 诊断增强 + 修掉一句带错方向的网络提示(2026-09-24,已发布)
**背景**:v2.3.0 修完材料中心默认源后,继续做"让只存在于用户手机上的状态可见"这件事 —— 内容源可达性、小组件同步状态,都取决于用户的网络与桌面,我这边看不到;用户遇到问题只能靠来回问答。本版把这两类状态摊进「诊断信息」。
**交付**:
- **「我的 → 诊断信息」新增两段**:①材料源整表(每个源一行:✔ 最近可用 · 3 分钟前 / ✖ 上次失败 · 2 小时前 —— 失败原因 / — 还没试过),②桌面小组件状态(已添加几个 · 最后同步 今天 08:12)。用户截一屏即可定性"材料中心为什么打不开""小组件为什么是旧的"
- **修误导性提示**:实测 BBC 的失败是 **TLS 握手被中断**(不是超时),而旧代码对"连接被中断"一律提示"大文件(公版书全文)首次抓取可能被中途掐断" —— 请求 RSS 列表却被暗示"整本书太大",排查方向被带偏。现在按 `longFetch` 分场景给提示;超时文案里的秒数也跟着实际超时走(全文 90 秒,不再写死 20 秒)
- **新增 `tool/probe_sources.dart` 端到端自检工具**:用 App 自己的抓取+解析代码跑"列表 → 抓正文 → 切块"(因为"能连通"≠"解析得出条目",源站改版会让列表静默变空)。实测结果:npr 列表 1.6s / 正文 1.7s(590 词)、arxiv 列表 1.2s / 正文 0.8s(276 词)、gutenberg 列表 0.9s 全部 OK;bbc_le TLS 握手中断、voa_le/ted/wikipedia 20 秒超时 —— 即**默认源 NPR 的"列表 → 正文"链路是通的**
**新增测试(3 例)**:诊断整表 2(含"每个源都必须出现在表里")、最后同步时间 1
**验证**:analyze **0 error / 0 warning**;**614 测试全绿 + 1 skip**(v2.3.0 为 611);APK 2.3.1+58 arm64,sha256 `05da21da…9f95` 与 Release asset `digest` 逐字一致;签名同 2.3.0 可直接覆盖安装

### v2.3.0 — 桌面小组件 + 材料中心可用性修复 + 结构整理(2026-09-24,已发布)
**背景**:v2.2.0 发到真机后用户实测反馈三件事:①「材料中心没法使用」;②「材料中心」与「其他输入材料」本质是同一个功能定位,应合并;③「导师」应移到「我的」右边并改名「学习助理」,而「AI 学习建议」完全没用要删掉。另外用户明确「跟读功能暂不需要」。本版按这四条 + 2.2 收尾的桌面小组件一起交付。
**交付**:
- **材料中心可用性修复(根因是默认值选错)**:旧实现默认选中列表第一个源 = BBC Learning English,而实测(2026-09-24)BBC / VOA / TED / Wikipedia 在中国大陆**全部超时**,NPR(2.7s)/ arXiv(0.9s)/ Gutenberg(26s,首包慢)可用 —— 于是用户打开材料中心的第一屏**必然是失败**,看起来像功能坏了。修法:默认源改 NPR;新增**内容源可用性记忆**(Hive 记每个源最近成功/失败 + 原因),下次默认落到上次成功过的源;源 chip 带可用性标记(✔ 最近可用 / ⚠ 上次失败)、选中源下方一行说明它是什么 + 在你这儿行不行;新增「**检测可用源**」一次串行探测全部源并给结论(手工试错一次要等 9~20 秒);失败卡片重写为"哪个源 + 为什么 + 换到 XXX"按钮;公版书全文超时单独放宽到 90 秒(实测 614KB 首包 26 秒,原 20 秒必然失败)
- **材料中心与其他输入材料合并**:材料中心新增「按你的水平找材料」分类入口(原 `ai_discovery_section.dart` 的网格并到这里),输入页删掉那张独立卡片与文件
- **底栏结构调整**:「导师」→「**学习助理**」并移到「我的」右边;顺序改为 **输入 / 输出 / 我的 / 学习助理**(`ReadFlowTab` 枚举与 `IndexedStack` 同步,所有用户可见文案里的「导师」一并改名);启动仍默认落在学习助理页
- **删除「AI 学习建议」**:入口、状态、provider 与 API 方法全部删除。它只有"生词收藏数 + 连续天数 + 按书分布"三个输入,产出的都是"先定个小目标、每天 10 分钟"这类套话,还要花一次付费 API 调用;这个定位已由学习助理(本地诊断 + 带数字依据 + 可打勾)真正实现
- **桌面小组件(v2.2 收尾)**:`home_widget` 数据通道 + 原生 `ReadFlowWidgetProvider`(RemoteViews 深色卡片)+ 文案规则纯函数层 + 「我的 → 桌面小组件」页(状态 / 一键添加 / 手动同步并预览 / 手动添加步骤)。显示:今天第一件该做的事 + 待复习数 + 新词额度 + 连续天数,点开进 App;**过期数据不冒充今天**(隔 ≥2 天改说"打开 App 刷新今日任务",原生侧每 30 分钟重绘时独立做同一判断);全新安装给引导而不是显示 0
  - 同口径:新词额度用复习页同一个 `FsrsScheduler.newWordQuota`;连续天数把 `LearnerSnapshotLoader.streakDays` 公开复用(桌面与 App 数字打架比不显示更糟)
- **跟读不做**(用户明确:暂不需要)。原判断仍成立:没有录音 + 语音识别就没有可信的发音评分,不做假分数
**新增测试(23 例)**:桌面小组件文案与过期规则 12、内容源可用性 11(含"默认源不能退回列表第一项"这条防回归断言)
**验证**:analyze **0 error / 0 warning**(24 infos);**611 测试全绿 + 1 skip**(v2.2.0 为 588);APK 2.3.0+57 arm64(24.1MB),本地 sha256 `fe1d8e97…c5b5` 与 Release asset `digest` **逐字一致**;已核对打包后清单里注册了小组件 provider(`…ReadFlowWidgetProvider` + `APPWIDGET_UPDATE`);签名同 2.2.0,可直接覆盖安装
**说明**:小组件的**外观**只能在你的桌面上验证(App 内的预览卡只还原文字与配色);材料中心的可达性结论来自本机实测(2026-09-24),你的网络若不同,点「检测可用源」即可得到你那边的结论

### v2.2.0 — 2.0 阶段三:听说与生产力(2026-09-23,已发布)
**背景**:2.0 的第三阶段。v2.1.0 解决了"记忆与反馈",本版解决**输入与产出的闭环**(听→写)、**数据主权**(材料也要能带出去)与**日常可读性**(深色模式)。设计依据见 `docs/PLAN-2.0.md`。
**交付**:
- **听力播放**:`just_audio` + `audio_session`(按语音内容配置会话,来电让路);全 App 单播放器 + 顶部播放条(±15 秒、倍速 0.75–1.5、可读中文错误 + 重试);播客/TED 材料边听边看稿,离开页面自动暂停
- **听写练习**:材料抽 5 句 → 系统 TTS 按句朗读(可慢速 0.35)→ 打字交卷 → **逐词 LCS 客观判分**(正确率/漏词/语序提示)+ 每句得分与一轮总评;漏词一键入生词本并进复习队列。**只判文字、不评发音**(没有 ASR 就不编造发音分,页面写明)
- **阅读包导出**:单篇材料 → Markdown(元信息[来源/链接/许可/难度/覆盖率/时长] + 正文分段 + 生词表);整库 → `exports/阅读包-<时间戳>/` 一篇一个文件;文件名安全化(路径字符/限长),失败给中文原因,绝不写空文件
- **深色模式**:light/dark 双主题(深底 #12141A、卡片 #1B1F27、输入框 #262B35,靠亮度分层;深色主色换亮蓝 #6BA8E8;实心按钮 = 亮底深字;正文 #E6E8EC 而非纯白)+ `ThemeController`(Hive 落盘、启动前读回避免首帧闪白)+ 新增「外观」页(跟随系统/浅色/深色 + 实时预览)
- **全仓硬编码清扫(35 文件约 170 处)**:次要文字→`onSurfaceVariant`、正文/单词释义→`onSurface`、图标→`onSurfaceVariant`、边框→`outlineVariant`、浅底填充→`surfaceContainerHighest`;淡彩面板(`blue[50]` 等)改**半透明色相**(浅色下观感不变、深色下不出现近白亮斑);只保留三类刻意写死的颜色(图片叠加角标、全屏遮罩、成绩/词型/掌握度等语义色)
**新增测试(25 例)**:听力 8、听写 12、阅读包 5、深色模式 8(含**算出来的 WCAG AA 对比度断言** + **硬编码闸门**:扫 lib/ 禁止灰阶极端档位与淡彩色阶再次出现)
**验证**:analyze **0 error / 0 warning**(24 infos,与 2.0 之前持平);**588 测试全绿 +1 skip**(v2.1.0 为 563);APK 2.2.0+56 arm64(23.1MB),本地 sha256 与 Release asset `digest` **逐字一致**(`f851831a…`)
**未做(明确留待)**:桌面小组件(需原生 AppWidget + 插件 + 真机反复验证,需先定内容)、跟读(需录音 + ASR,不做假评分)、每日提醒(通知包未获批)
**过程记录**:深色清扫用 5 个并行子代理按"语义替换表"改 35 个文件,它们各自跑 `dart analyze` 自查(0 error),汇总后由我复核 diff、补漏(裸 `Colors.grey`、淡彩面板、`scroll_buttons` 白底圆钮、过期注释)并以 `theme_test` 闸门兜底;期间发现两处需要判断的语义分叉(流式正文该用 onSurface、进度条槽该用 surfaceContainerHighest),已按"正文要高对比"的原则定案。

### v2.1.0 — 2.0 阶段二:记忆与反馈(2026-09-23,已发布)
**背景**:v2.0.0 交付了"地基 + 导师 + 材料";本版补齐**记忆与反馈** —— 复习不再靠用户自己安排,错误不再靠用户自己回想。审查与设计依据见 `docs/PLAN-2.0.md`。
**交付**:
- **复习页接 FSRS**:今日队列(过期最久 + 记忆最脆弱优先、复习优先的新词配额)+ **四档评分**(不认识/模糊/认识/太简单,每次更新稳定度/难度/到期)+ 未来 7 天负荷预测与建议 + 完成小结;答"不认识"自动进错误档案;**三种问法**(认词/拼写/听写,判分忽略大小写/空白/连字符)
- **错误档案页**:按考点聚合(次数/首末时间/来源/样例)、标记已改正与恢复、复发红色角标、两套空态
- **每周报告**:本地生成(滚动 7 天)的概览/亮点/建议/负荷条形图,可复制为文本;亮点与建议每条必带数字
- **配额管理**:学习偏好页设每日分钟数与每日新词上限(含"只复习不加新词"),驱动复习队列与导师任务卡
- **听力播放**(提前交付):`just_audio` + `audio_session`,顶部播放条(±15s/倍速 0.75-1.5/可读错误+重试),播客材料边听边看稿;全 App 单播放器,离开页面自动暂停
- **导出与备份**(提前交付):完整备份 JSON(含 FSRS 状态与画像,**不含 API Key**)、生词本 CSV、Anki TSV;合并/覆盖两种恢复;文件写 `Android/data/<包名>/files/exports/`
**修掉的真 bug**:① 学习偏好页只存音色、题材/关键词黑名单从未落库(用户选的屏蔽一直没生效);② 导出复习状态用 24h 窗口截断导致差一天(改自然日口径);③ 新建库缺 `phonetic_uk/us` 列(靠自检补列,现已写进建表语句)
**验证**:analyze 0 error / 0 warning(24 infos);**563 测试全绿 +1 skip**(v2.0.0 为 505);APK 2.1.0+55 arm64,sha256 与 Release asset `digest` 逐字一致
**未做(留 v2.2 收尾)**:逐句听写/跟读(需时间轴,计划做手动选段循环、不做假评分)、桌面小组件、深色模式、每日提醒(通知包未获批)


### v2.0.0 — 2.0 阶段一:地基 + AI 导师 + 材料中心(2026-09-22,已发布)
**背景**:用户提出"2.0 大迭代"——要把它从"工具集 + 一次性 AI 文本"做成"能真正指导学习"的专业软件;并点名两个形同虚设的功能(**AI 学习建议**、**其他输入材料**)。经两轮讨论确认方向后按 `docs/PLAN-2.0.md` 分三阶段实施,本版是阶段一。
**交付**:
- **地基**:内嵌 5 万词频表 + 干净 1 万抽样池(离线零成本);**本地难度引擎**(已知词覆盖率/生词密度/CEFR/Flesch/预计时长/最该先学的词);**双模式词汇量测试**(速测 76 题 / 完整版 128 题 + 语法 5 + 阅读 3,伪词校准自评偏差,给区间与"你在哪一档开始掉");**学习者模型**(每个字段带来源(自报/测量/推断)+置信度,并记录"还缺什么")
- **AI 导师 tab**(底栏改四栏:导师/输入/输出/我的,默认落在导师页):诊断结论**本地规则算**(11 类,每条必须带依据数字)+ 今日任务卡(落库/可跳转/可打勾回填)+ 会话(带结构化证据包,硬性禁止编造)+ 画像缺口即问即存并重跑诊断
- **材料中心**:7 个公开内容源(全部无需 API Key;Gutenberg 公版书全文按章切块)+ 打开即本地算难度(读前卡)+ 阅读器(点词即查、一键收词入复习队列、进度/用时/查词落库)+ **读后测验**(材料原文本地生成完形填空与中译英,零 API、可复现,错词回复习队列并进错误档案)+ 粘贴自备材料 + 材料库按"今天最适合读"排序
- **英式/美式双音标**显示 + **TTS 音色切换**(学习偏好页)+ **题材/关键词黑名单**(推荐生成前过滤入库、展示层对缓存再过滤、导师选材同步遵守)
- **dbVersion 11**:10 张 v2.0 明细表(word_review/materials/material_content/material_progress/reading_sessions/quiz_results/error_tags/tutor_tasks/tutor_messages/tutor_memory)+ 双音标列 + 旧生词按掌握度初始化复习状态(保守到期,不设"永不到期")
**提前落地的 v2.1 地基**:FSRS-4.5 调度内核(稳定度/难度/可提取性/间隔/负荷预测/复习优先配额)+ 今日复习队列(到期排序 + 新词配额;**修正"从未复习的词不算到期"**)
**验证**:analyze 0 error / 0 warning(25 infos);**505 测试全绿 +1 skip**(v1.9.1 为 195);APK 2.0.0+54 arm64,sha256 与 Release asset `digest` 逐字一致
**未做(留 v2.1)**:复习页接 FSRS 四档评分与队列 UI、错误档案页、周报、每日提醒、听力播放器、导出/备份
**过程教训**:材料源解析层首轮实现漏了"实体解码进制分支/多 script 块/锚点正则回溯/RSS link 捕获组"四类真实缺陷 —— 由单测与真实网络实测抓出;`profile_home` 中途出现过一次半成品 error(子代理写入中间态),已在合并前拦下。
