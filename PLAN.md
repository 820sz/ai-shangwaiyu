...
## 状态
- 当前版本:**v1.2.16**(commit 待填,Release 已发 = Latest,带 app-release.apk)
- 当前阶段:🔵 修复发布 → 待真机验证(思考中档实际耗时 + 短语/句子完整显示 + 新 logo)
- 状态协议:DONE_WITH_CONCERNS(词条截断根因=模型 word 词条化截断+original_sentence 可选缺填;思考中档真实耗时待用户报数)
...
## 验证目标(当前版)
- [x] flutter analyze 0 error / 40 测试全绿(v1.2.16)
- [ ] **真机**:思考中档(中·约15s)实际耗时——不再触发降级重跑,一次性等结果
- [ ] **真机**:新识别一批,短语/句子完整显示(提示词强制 + 回退双保险)
- [ ] **真机**:新 logo 效果

## 当前任务 / 待办
- [ ] 等用户真机验证 v1.2.16(思考中档耗时 + 短语/句子完整显示)
- [ ] 若中档仍 >30s:查用户配置模型 + 考虑档位重映射(低=minimal/中=minimal?需用户定夺)
- [ ] 发布流程补"桌面复制 APK":最新版装完桌面复制一份存档
