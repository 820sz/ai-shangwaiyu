...
## 状态
- 当前版本:**v2.3.1+58**(Release v2.3.1 = Latest;上版 v2.3.0)
- 当前阶段:🔧 **2.0 三阶段全部交付**;v2.3.x 起进入"真机反馈驱动"的修整期 —— 2.3.0 修材料中心可用性 + 合并入口 + 底栏改名挪位 + 删 AI 学习建议 + 桌面小组件;2.3.1 把"只在用户手机上才看得见的状态"(材料源可用性、小组件同步)摊进「诊断信息」。剩余仅「每日提醒」(需通知包,用户未定);**跟读按用户要求不再排期**
- 状态协议:IN_PROGRESS(等用户真机复测:材料中心可用性、入口合并、底栏改名、桌面小组件、诊断页新增两段;当前分支 analyze 0 error/0 warning、614 测试全绿 +1 skip、v2.3.1 已发 Release)
